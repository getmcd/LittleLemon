...
#     def setUp(self):
#         # Create a user and authenticate (your view requires IsAuthenticated)
#         self.user = User.objects.create_user(username="tester", password="pass1234!")
#         self.client.login(username="tester", password="pass1234!")  # Session auth
...
#     def test_getall(self):
#         # If you don't have named URLs, hardcode the path:
#         response = self.client.get("/api/menu-items/")

#         self.assertEqual(response.status_code, status.HTTP_200_OK)
#         self.assertEqual(len(response.data), 2)
#         self.assertEqual(response.data[0]["title"], "Burger")   # ordered by title
#         self.assertEqual(response.data[1]["title"], "IceCream")
...
#         r = self.client.post("/api/menu-items/", payload, format="json")
